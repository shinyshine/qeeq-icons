...
function generateReactIcons(options) {
    ...
    util_1.series(clean_1.default([outputPath]), util_1.parallel(util_1.promisfy(exports.generateReactIcons$1({
        ...
        template: fs_1.readFileSync(path_1.join(__dirname, `./templates/${typescript ? 'ts/icon.tsx.ejs' : 'js/icon.jsx.ejs'}`), 'utf-8')
    })), util_1.promisfy(copy_1.default({
        fromDir: [
            path_1.join(__dirname, `./templates/${typescript ? 'ts/*.ts' : 'js/*.js'}`),
            path_1.join(__dirname, `./templates/${typescript ? 'ts/*.tsx' : 'js/*.jsx'}`),
